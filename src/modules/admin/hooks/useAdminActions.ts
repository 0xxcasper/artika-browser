...
export function useAdminActions({
  ...
  const handleMarkAsReadSimple = useCallback(
    async (submission: TourSubmission) => {
      try {
  ...
  const handleSubmitNote = useCallback(
    async (note: string, submission: TourSubmission) => {
      if (!note.trim()) {
  ...
  const handleMarkAllAsRead = useCallback(async () => {
    try {
      const unreadSubmissions =
        tourSubmissions?.filter((sub) => !sub.read) || [];
      await Promise.all(
        unreadSubmissions.map((sub) =>
          updateTourDocument(sub.id, {
            read: true,
  ...
  const handleNewsletterDelete = useCallback(
    async (subscriptionId: string) => {
      if (
        !confirm(
          'Are you sure you want to delete this newsletter subscription?',
        )
      )
        return;
  ...
  const handleNewsletterAddNote = useCallback(
    async (note: string, subscription: NewsletterSubscription) => {
      if (!note.trim()) {
        throw new Error('Note cannot be empty');
      }

      try {
        await NewsletterSubscriptionService.updateSubscriptionWithNote(
          subscription.id,
          note.trim(),
        );
        await refetchActiveNewsletterCount();
