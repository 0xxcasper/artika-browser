...
const defaultScheduleTourForm: ScheduleTourFormData = {
  ...
  schedule_tour_description:
    'Artika đã mở cửa giới hạn cho bạn đến tham quan trải nghiệm. Để lại thông tin để khôi nguồn chuyến đi đến miền bất tận.',
  ...
  schedule_tour_success_message:
    'Thank you! We will contact you soon to confirm your tour schedule.',
...
function extractScheduleTourFormData(doc: any): ScheduleTourFormData {
  return {
    schedule_tour_title:
      doc.data.schedule_tour_title ||
      defaultScheduleTourForm.schedule_tour_title,
    schedule_tour_description:
      doc.data.schedule_tour_description?.[0]?.text ||
      defaultScheduleTourForm.schedule_tour_description,
    schedule_tour_phone_placeholder:
      doc.data.schedule_tour_phone_placeholder ||
      defaultScheduleTourForm.schedule_tour_phone_placeholder,
    schedule_tour_email_placeholder:
      doc.data.schedule_tour_email_placeholder ||
      defaultScheduleTourForm.schedule_tour_email_placeholder,
    schedule_tour_date_placeholder:
      doc.data.schedule_tour_date_placeholder ||
      defaultScheduleTourForm.schedule_tour_date_placeholder,
    schedule_tour_button_text:
      doc.data.schedule_tour_button_text ||
      defaultScheduleTourForm.schedule_tour_button_text,
    schedule_tour_success_message:
      doc.data.schedule_tour_success_message ||
      defaultScheduleTourForm.schedule_tour_success_message,
    schedule_tour_error_message:
      doc.data.schedule_tour_error_message ||
      defaultScheduleTourForm.schedule_tour_error_message,
    schedule_tour_validation_messages: {
      schedule_tour_phone_required:
        doc.data.schedule_tour_validation_messages?.[0]
          ?.schedule_tour_phone_required ||
        defaultScheduleTourForm.schedule_tour_validation_messages
          .schedule_tour_phone_required,
...
function extractNewsletterFormData(doc: any): NewsletterFormData {
  return {
    newsletter_title:
      doc.data.newsletter_title || defaultNewsletterForm.newsletter_title,
    newsletter_email_placeholder:
      doc.data.newsletter_email_placeholder ||
      defaultNewsletterForm.newsletter_email_placeholder,
    newsletter_button_text:
      doc.data.newsletter_button_text ||
      defaultNewsletterForm.newsletter_button_text,
    newsletter_success_message:
      doc.data.newsletter_success_message ||
      defaultNewsletterForm.newsletter_success_message,
    newsletter_error_message:
      doc.data.newsletter_error_message ||
      defaultNewsletterForm.newsletter_error_message,
    newsletter_validation_message:
      doc.data.newsletter_validation_message ||
      defaultNewsletterForm.newsletter_validation_message,
  };
}
...
function extractFooterData(doc: any): FooterData {
  return {
    footer_contact_title:
      doc.data.footer_contact_title || defaultFooterData.footer_contact_title,
    footer_contact_address:
      doc.data.footer_contact_address?.map((item: any) => ({
        address_line: item.address_line || '',
      })) || defaultFooterData.footer_contact_address,
    footer_contact_info:
      doc.data.footer_contact_info?.map((item: any) => ({
        contact_item: item.contact_item || '',
      })) || defaultFooterData.footer_contact_info,
    footer_artika_title:
      doc.data.footer_artika_title || defaultFooterData.footer_artika_title,
    footer_social_title:
      doc.data.footer_social_title || defaultFooterData.footer_social_title,
    footer_social_links: {
      instagram_url:
        doc.data.footer_social_links?.[0]?.instagram_url ||
        defaultFooterData.footer_social_links.instagram_url,
      facebook_url:
    ...
    footer_privacy_policy: {
      privacy_policy_text:
        doc.data.footer_privacy_policy?.[0]?.privacy_policy_text ||
        defaultFooterData.footer_privacy_policy.privacy_policy_text,
      privacy_policy_link:
...
export async function fetchScheduleTourForm(
  locale: string,
